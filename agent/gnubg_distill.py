...
def generate_training_data(
    ...
    *,
    ...
    """Collect (features[198], gnubg P(win)) pairs from self-play rollouts.

    Each visited position is encoded from the side-to-move's perspective and
    labelled with gnubg's win probability for that side. `policy="random"`
    plays uniformly-random legal moves (fast, wide coverage); the teacher
    label is gnubg's regardless of how the position was reached.
    """
    ...
    while len(feats) < n_positions:
        ...
        for _ in range(max_plies):
            if state.terminal():
                break
            to_move = state.turn
            board = Board(tuple(state.board), tuple(state.bar), tuple(state.off))
            b0, b1 = board_to_tanboard(board, to_move)
            out, _ = evaluator.evaluate(b0, b1)
            feats.append(encode_full_board(state.board, state.bar, state.off, to_move))
            labels.append(out[0])  # P(win) for the side to move
...
def main() -> None:
    ...
    X, y = generate_training_data(args.positions, wd_path=args.wd_path, seed=args.seed)
