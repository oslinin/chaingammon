...
- **`agent/coach_dialogue.py` + `coach_service.POST /chat`** — Phase A of the human↔agent dialogue (see [docs/coach-dialogue.md](docs/coach-dialogue.md)). New data shapes (`DialogueMessage`, `DialogueState`, `ChatRequest`, `ChatResponse`), prompt-assembly helper (`build_chat_prompt`) with per-kind framing for `open_turn` / `human_reply` / `move_committed`, and a per-session keyword-driven preference signal (`update_preferences`) clipped to `[-1, 1]`. The `/chat` endpoint runs the full pipeline (prompt assembly → LLM → preference update → delta) with a deterministic stub for the LLM call so the frontend can integrate against the response shape; Phase B replaces the stub with Qwen 2.5 7B on 0G Compute (and flan-t5-base local fallback). 23 dialogue-shape tests + 8 endpoint tests.
- **`docs/coach-dialogue.md`** — five-phase design plan for turn-by-turn human↔agent collaboration: per-turn dialogue, agent reads the human, human can talk back, feedback adjusts the coach, bounded latency. Documents data shapes, conversation flow, where each piece lives, cost/latency budget, explicit non-goals.
