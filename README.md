...
The career-mode extras head encodes a 40-d context vector — `[own_18 | opp_18 | stake | tournament | is_team | bias]` — fed into a separate MLP head alongside the 198-d board encoding. Each agent sees both its own accumulated style (slots [0:18]) and the opponent's public style (slots [18:36]) at every forward pass, so a single checkpoint handles diverse opponents without retraining. Stake size is log1p-scaled into slot [36]; tournament position in [37]; team-match flag in [38]; bias constant 1.0 in [39]. The 18 style axes are `ACTIVE_AXES = CATEGORIES[:18]` — all non-cube categories from `agent_overlay.CATEGORIES`; the two cube-offer / cube-take axes are excluded because they have no v1 move classifier. Opponent-style profiles stored on 0G Storage (the per-agent style KV) drop straight in via `encode_career_context` at inference time.
...
| `--career-mode`                               | Sample a fresh `CareerContext` per match (uniform own/opponent style, log-uniform stake). Requires `--extras-dim >= 40`. Updates both agents' style overlays after each game via neutral EMA (exposure-based, no win/loss bias).                                                                       |
