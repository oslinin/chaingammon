...
interface AgentTeammateMessage {
  role: "human" | "agent";
  text: string;
}

interface AgentTeammateResponse {
  reply: string;
...
export function AgentTeammatePanel({
  ...
  const [dialogue, setDialogue] = useState<AgentTeammateMessage[]>([]);
  const [strategyInput, setStrategyInput] = useState("");
  const [sending, setSending] = useState(false);

  // Last Chief-of-Staff response for the recommendation highlight + deep-dive.
  const [lastResponse, setLastResponse] = useState<AgentTeammateResponse | null>(null);
  ...
  const sendStrategy = async (text: string) => {
    ...
    const userMsg: AgentTeammateMessage = { role: "human", text: text.trim() };
    ...
    try {
      ...
      const data = (await res.json()) as AgentTeammateResponse;
