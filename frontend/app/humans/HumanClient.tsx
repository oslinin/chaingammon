...
function HumanClientInner() {
  ...
  const rawId = mounted ? (searchParams.get("id") ?? "") : "";
  ...
  useEffect(() => {
    ...
    const computeFromBlock = async (): Promise<bigint | "earliest"> => {
      ...
      if (typeof active?.deployedBlock === "number") return BigInt(active.deployedBlock);
    ...
    const scan = async () => {
        const eventDef = {
          ...
          name: "SubnameMinted",
          inputs: [
            { type: "string" as const, name: "label", indexed: false },
            { type: "bytes32" as const, name: "node", indexed: true },
            { type: "address" as const, name: "subnameOwner", indexed: true },
            { type: "uint256" as const, name: "inftId", indexed: false },
          ],
        };

        const fromBlock = await computeFromBlock();
        if (fromBlock === "earliest") {
          ...
          const match = logs.find(l => (l.args as any).label === rawId);
          if (match && (match.args as any).subnameOwner) {
            setResolvedAddress((match.args as any).subnameOwner as `0x${string}`);
          }
          return;
        }
...
export default function HumanClient() { return <Suspense fallback={<div className="flex min-h-screen bg-zinc-50 dark:bg-black p-8 text-zinc-500">Loading...</div>}><HumanClientInner /></Suspense>; }
