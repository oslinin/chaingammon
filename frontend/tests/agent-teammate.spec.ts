...
// Agent Teammate panel.  Mirrors the pattern used by dice-size.spec.ts and
...
test("Agent Teammate panel renders header label", async ({ page }) => {
  await page.goto(FIXTURE);
  await expect(page.locator("text=Agent Teammate").first()).toBeVisible({ timeout: 8000 });
});
...
test("Agent Teammate panel renders strategy input", async ({ page }) => {
  await page.goto(FIXTURE);
...
test("Agent Teammate panel renders Ask button", async ({ page }) => {
  await page.goto(FIXTURE);
  await expect(page.locator("button", { hasText: "Ask" })).toBeVisible({ timeout: 8000 });
});

test("Agent Teammate panel renders 'Top moves this turn' label when loading", async ({ page }) => {
  await page.goto(FIXTURE);
