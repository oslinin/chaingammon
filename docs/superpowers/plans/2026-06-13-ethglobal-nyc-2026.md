...
- **PR 1.6** — **Demo visibility (autonomous signing is now visible in the UI):**
    - `AgentWalletPanel.tsx` auto-provisions on mount (no "Load wallet" click), polls `GET
      /agents/{id}/privy-wallet` every 5 s so the balance ticks up live after a match settles.
    - Auth key badge: green "✓ enabled" when registered, amber "Enable" button otherwise —
      clicking it calls `POST /agents/{id}/privy-wallet/auth-key` live during the demo.
    - Last autonomous tx row: amount, time-ago, and Etherscan link to the on-chain tx.
    - `POST /agents/{id}/privy-wallet` now auto-calls `register_auth_key` on first provision.
    - New `GET /agents/{id}/privy-wallet` (read-only poll) and
      `POST /agents/{id}/privy-wallet/auth-key` (explicit enable) endpoints.
    - `deposit_dividend_via_privy(agent_id, usdc_amount, privy_wallets)` on `ChainClient`:
      agent signs `approve` + `depositDividend` via Privy RPC instead of the server operator key.
